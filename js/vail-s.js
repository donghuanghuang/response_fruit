...
function bindvail(obj, reg, name, msg1, msg2) {
    obj.on("focus keyup", function () {
        if (reg.test($(this).val())) vailData($(this), name, $(this).val(), msg1);else $(this).parent().next().find("span").html(msg2).attr('class', 'text-info');
    });
}
...
function vailData(obj, datname, dat, msg) {
    $.ajax({
        ...
        success: function success(result) {
            console.log(result);
            if (result == "success") obj.parent().next().find("span").html("验证通过").attr('class', 'text-success');else obj.parent().next().find("span").html(msg).attr('class', 'text-danger');
        }
    });
}

$("#pwd").on("focus keyup", function () {
    if (/^\w{6,12}$/.test($(this).val())) $(this).parent().next().find("span").html("验证通过").attr('class', 'text-success');else $(this).parent().next().find("span").html("密码必须6-12位").attr('class', 'text-danger');
});

$("#cpwd").on("focus keyup", function () {
    var pwd = $("#pwd").val();
    if (pwd == $(this).val() && $(this).val() != "") $(this).parent().next().find("span").html("验证通过").attr('class', 'text-success');else $(this).parent().next().find("span").html("两次输入密码不一致").attr('class', 'text-danger');
});
    ...
    if ($(".text-success").length == 5 && $("#checkb").is(':checked')) {
        ...
        $.ajax({
            ...
            success: function success(data) {
                if (data == "success") location.href = "login.html";else alertMsg("注册失败", "注册失败", false, "关闭", "关闭", function () {});
            }
        });
    } else {
...
//# sourceMappingURL=vail-s.js.map